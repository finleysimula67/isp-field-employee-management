...
export default function DeleteConfirmModal({ open, title, message, count, onConfirm, onCancel, loading }: DeleteConfirmModalProps) {
  if (!open) return null
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onCancel}>
      <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md mx-4" onClick={e => e.stopPropagation()}>
        ...
        {count && count > 1 ? (
          <p className="text-sm font-medium text-red-600">{count} records will be deleted.</p>
        ) : null}
