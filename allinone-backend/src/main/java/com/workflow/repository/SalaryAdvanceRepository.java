...
public interface SalaryAdvanceRepository extends JpaRepository<SalaryAdvance, Long> {
    @Query("SELECT s FROM SalaryAdvance s WHERE s.employee.id = :employeeId AND s.deletedAt IS NULL ORDER BY s.requestDate DESC")
    List<SalaryAdvance> findByEmployeeIdOrderByRequestDateDesc(@Param("employeeId") Long employeeId);

    @Query("SELECT s FROM SalaryAdvance s WHERE s.employee.id = :employeeId AND s.isSettled = false AND s.status IN :statuses AND s.deletedAt IS NULL")
